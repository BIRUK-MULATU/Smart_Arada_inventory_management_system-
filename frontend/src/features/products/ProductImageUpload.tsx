...
export function ProductImageUpload({ product, onUpdated }: ProductImageUploadProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-sm font-medium text-ink-700">Photo</span>
      <div className="flex flex-col items-center gap-3">
        {product.imageUrl ? (
          <ZoomableImage
            src={product.imageUrl}
            alt={product.name}
            className="h-48 w-48 rounded-lg border border-ink-200"
          />
        ) : (
          <div className="flex h-48 w-48 items-center justify-center rounded-lg border border-dashed border-ink-300 text-center text-sm text-ink-400">
            No photo
          </div>
        )}
        <div className="flex w-full flex-col items-center gap-1">
          <label className="min-h-11 w-full cursor-pointer rounded-md border border-ink-300 px-3 py-2 text-center text-sm font-medium text-ink-700 hover:bg-ink-50">
