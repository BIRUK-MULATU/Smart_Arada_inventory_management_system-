...
export function RecentActivity({ summary }: { summary: DashboardSummary }) {
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <Card>
        <h2 className="mb-2 text-sm font-medium text-ink-600">Recent sales</h2>
          ...
          <ul className="divide-y divide-ink-100">
            {summary.recentSales.map((sale) => (
              <li key={sale.id} className="flex items-center justify-between py-2 text-sm">
                <span className="text-ink-900">{sale.employeeName}</span>
                <span className="text-ink-600">${sale.totalAmount.toFixed(2)}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <Card>
        <h2 className="mb-2 text-sm font-medium text-ink-600">Recent inventory movements</h2>
          ...
          <ul className="divide-y divide-ink-100">
            {summary.recentInventoryMovements.map((movement) => (
              <li key={movement.id} className="flex items-center justify-between py-2 text-sm">
                <span className="text-ink-900">{movement.productName}</span>
                <span className={movement.type === "STOCK_IN" ? "text-green-700" : "text-ink-600"}>
