  ...
  return (
    <Card>
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-sm font-medium text-slate-600">Sales and revenue over time</h2>
        <div className="flex rounded-md border border-slate-300 text-xs">
          {GRANULARITIES.map((option, index) => (
            <button
              ...
              className={`min-h-9 px-3 py-1 font-medium ${index > 0 ? "border-l border-slate-300" : ""} ${
                granularity === option.value ? "bg-slate-900 text-white" : "bg-white text-slate-700 hover:bg-slate-50"
              }`}
            >
              {option.label}
            </button>
