...
export function TopProductsList({ products }: { products: TopProduct[] }) {
  return (
    <Card>
      <h2 className="mb-2 text-sm font-medium text-slate-600">Top products this period</h2>
        ...
        <ol className="divide-y divide-slate-100">
          {products.map((product, index) => (
            <li key={product.productId} className="flex items-center justify-between py-2 text-sm">
              ...
              <span className="text-slate-600">{product.unitsSold} units · ${product.revenue.toFixed(2)}</span>
            </li>
          ))}
        </ol>
      )}
